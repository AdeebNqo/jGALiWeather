...
public class CP22 {
    ...
    public void test() {
        ...
        try {
            ...
            FogGenerator nssg = new FogGenerator(f_output, tr.getLabelsets().get("FOG"), tr.getLabelsets().get("PD"), tr.getLabelsets().get("DW"), Calendar.getInstance(), curr_var.getActual_data_length(), nlgFactory);
            
            String salida = realiser.realiseSentence(nssg.generate()).replaceAll("\\s+", " ").trim();
            
            Assert.assertEquals("There will be morning fog on Wednesday probably persistent, on Friday probably persistent and on Saturday, and night fog on Saturday.", salida);
        } catch (Exception ex) {
            Logger.getLogger(CP22.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
}
