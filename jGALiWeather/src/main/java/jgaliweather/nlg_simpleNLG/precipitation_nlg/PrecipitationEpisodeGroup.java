...
public class PrecipitationEpisodeGroup {
    ...
    public SPhraseSpec generateReport(HashMap<String, LabelSet> template_labels) {
        ...
        for (int i = 0; i < episodes.size(); i++) {
            episodes_list.addCoordinate(episodes.get(i).toText(template_labels.get("RNLGE"), template_labels.get("R"), template_labels.get("DW"), template_labels.get("PD"), "TD"));
        }
    ...
    private SPhraseSpec addTextNuances(HashMap<String, LabelSet> template_labels) {
        if (nuances.size() > 0) {
            ...
            for (int i = 0; i < nuances.size(); i++) {
                nuances_list.addCoordinate(nuances.get(i).toText(template_labels.get("RNLGE"), template_labels.get("R"), template_labels.get("DW"), template_labels.get("PD"), "TD"));
            }
