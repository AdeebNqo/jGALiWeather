...
public class PrecipitationNuance {
     ...
     */
    public AdjPhraseSpec toText(LabelSet expresion_template, LabelSet label_template, LabelSet day_template, LabelSet time_template, String mode) {

        AdjPhraseSpec phrase = nlgFactory.createAdjectivePhrase(label_template.getLabels().get(label).getData());

        ArrayList<PPPhraseSpec> interval = duration.toText(expresion_template, day_template, time_template, mode);
