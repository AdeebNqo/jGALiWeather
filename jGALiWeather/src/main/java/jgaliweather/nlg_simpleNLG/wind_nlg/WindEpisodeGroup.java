...
public class WindEpisodeGroup {
    ...
    public CoordinatedPhraseElement generateReport(HashMap<String, LabelSet> template_labels) {
        ...
        CoordinatedPhraseElement changes = episodes.get(0).addTextChanges(template_labels.get("WNLGE"), template_labels.get("W"), template_labels.get("DW"), template_labels.get("PD"));
        if (changes != null) {
        ...
        if (episodes.size() > 1) {
            for (int i = 0; i < episodes.size() - 2; i++) {
                ...
                CoordinatedPhraseElement changes_aux = episodes.get(i + 1).addTextChanges(template_labels.get("WNLGE"), template_labels.get("W"), template_labels.get("DW"), template_labels.get("PD"));
                if (changes_aux != null) {
            ...
            CoordinatedPhraseElement changes_aux = episodes.get(episodes.size() - 1).addTextChanges(template_labels.get("WNLGE"), template_labels.get("W"), template_labels.get("DW"), template_labels.get("PD"));
            if (changes_aux != null) {
                changes_aux.setFeature(Feature.APPOSITIVE, true);
                windCondition_aux.addPostModifier(changes_aux);
            }
