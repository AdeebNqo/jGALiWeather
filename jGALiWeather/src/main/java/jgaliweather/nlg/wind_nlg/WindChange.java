...
public class WindChange {
     ...
     */
    public ArrayList<PPPhraseSpec> toText(LabelSet label_template, LabelSet day_template, LabelSet time_template) {

        ArrayList<PPPhraseSpec> elements = new ArrayList();
        elements.add(nlgFactory.createPrepositionPhrase("to", label_template.getLabels().get(label).getData()));
        elements.addAll(duration.toText(day_template, time_template, "N"));
