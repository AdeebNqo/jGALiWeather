...
public class WindEpisode {
     ...
     */
    public ArrayList<NLGElement> toText(LabelSet label_template, LabelSet day_template, LabelSet time_template) {
        ...
        if (!changes.isEmpty()) {
            ...
            phrases.addAll(duration.toText(day_template, time_template, "START"));

        } else {
            phrases.add(nlgFactory.createAdjectivePhrase(label_template.getLabels().get(label).getData()));
            phrases.addAll(duration.toText(day_template, time_template, "N"));
        }
     ...
     */
    public CoordinatedPhraseElement addTextChanges(LabelSet label_template, LabelSet day_template, LabelSet time_template) {
        ...
        if (changes.size() > 0) {
            ...
            VPPhraseSpec first = nlgFactory.createVerbPhrase("change");
            first.setFeature(Feature.FORM, Form.GERUND);
            ArrayList<PPPhraseSpec> change_text_list = changes.get(0).toText(label_template, day_template, time_template);
            ...
            if (changes.size() > 1) {
                for (int i = 0; i < changes.size() - 2; i++) {
                    ArrayList<PPPhraseSpec> aux_list = changes.get(i + 1).toText(label_template, day_template, time_template);
                ...
                ArrayList<PPPhraseSpec> aux_list = changes.get(changes.size() - 1).toText(label_template, day_template, time_template);
                SPhraseSpec aux_phrase = nlgFactory.createClause();
                for (PPPhraseSpec prep : aux_list) {
                    aux_phrase.addComplement(prep);
                }
